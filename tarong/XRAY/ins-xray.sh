...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/JerrySBG/scvps/izin > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/JerrySBG/scvps/izin | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/JerrySBG/scvps/izin | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
wget -q -O /usr/bin/bbr "https://raw.githubusercontent.com/Tarap-Kuhing/tarong/main/tarong/SSH/bbr.sh"
chmod +x /usr/bin/bbr
bbr >/dev/null 2>&1
...
sleep 1
wget -q -O /usr/bin/auto-set "https://raw.githubusercontent.com/Tarap-Kuhing/tarong/main/tarong/XRAY/auto-set.sh" && chmod +x /usr/bin/auto-set 
